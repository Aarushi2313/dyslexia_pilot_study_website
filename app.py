...
import difflib
import static_ffmpeg  # Import to setup ffmpeg paths automatically
...
def calculate_typing_metrics(text, keystrokes_data, reference_text=None):
    ...
    try:
        # 1. Spelling Error Rate (Using Reference Text as Ground Truth)
        words = re.findall(r"[a-zA-Z]+", text.lower())
        
        if reference_text:
            # Create a set of valid words from the reference text
            valid_words = set(re.findall(r"[a-zA-Z]+", reference_text.lower()))
            if words and valid_words:
                errors = sum(1 for word in words if word not in valid_words)
                metrics['spelling_error_rate'] = (errors / len(words)) * 100
        else:
            # Fallback if no reference text provided (e.g. creative writing)
            # Since we removed the dictionary, we can't check spelling without reference
        ...
        # 3. Edit Density and Keystroke Analysis
        if isinstance(keystrokes_data, str):
            try:
                keystrokes = json.loads(keystrokes_data)
            except:
                keystrokes = []
        else:
...
def ensure_video_recordings_table():
    """Create video_recordings table if it doesn't exist."""
    conn = connect_db()
    if not conn:
        return
    try:
        ...
        print(f"Error ensuring video_recordings table: {e}")
    finally:
        try:
            cursor.close()
            conn.close()
...
def calculate_reading_accuracy(ground_truth, transcribed_text):
    """
    Compares transcribed text with ground truth to calculate accuracy metrics.
    """
    if not ground_truth:
        return {
            "accuracy": 0, "skipped_words": 0, 
            "mispronounced_words": 0, "extra_words": 0
        }
    
    if not transcribed_text:
         # If no transcription, assume 0 accuracy but we can't count errors accurately 
         # other than saying everything was skipped.
         # But wait, silence is different. Let's return 0s.
         return {
            "accuracy": 0, "skipped_words": len(ground_truth.split()),
            "mispronounced_words": 0, "extra_words": 0
        }
    ...
    matcher = difflib.SequenceMatcher(None, gt_words, trans_words)
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if tag == 'equal':
            ...
            # Text in GT that is missing in Trans
            skipped_words += (i2 - i1)
        elif tag == 'insert':
             # Text in Trans that is not in GT
            extra_words += (j2 - j1)
        elif tag == 'replace':
            # Text in GT that was recognized as something else
            mispronounced_words += (i2 - i1)
            
    total_gt_words = len(gt_words)
    accuracy = (correct_words / total_gt_words) * 100 if total_gt_words > 0 else 0
...
def create_task_html(task_name, instructions, estimated_time, devices_required, example, main_content):
    ...
    # Generate content that extends base_dashboard.html
    html_content = f"""{{% extends "base_dashboard.html" %}}

{{% block title %}}{task_name} - Dyslexia Research Study{{% endblock %}}

{{% block content %}}
<div class="content-wrapper mt-4 sm:mt-6 md:mt-8 mb-8 sm:mb-12 md:mb-16 px-3 sm:px-4">
    <!-- Task Info Popup -->
    <div id="task-info-popup" class="max-w-4xl mx-auto bg-white rounded-xl shadow-lg p-6 md:p-8 mb-6">
        <h1 class="text-2xl md:text-3xl font-bold text-[#1A237E] mb-4">{task_name}</h1>
        <div class="space-y-4 text-gray-700">
            <div class="bg-blue-50 p-4 rounded-lg border border-blue-100">
                <h3 class="font-bold text-lg mb-2 text-blue-800">Instructions</h3>
                <p>{instructions}</p>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="bg-gray-50 p-4 rounded-lg">
                    <span class="font-semibold block text-gray-900">Estimated Time</span>
                    <span>{estimated_time} minutes</span>
                </div>
                <div class="bg-gray-50 p-4 rounded-lg">
                    <span class="font-semibold block text-gray-900">Devices Required</span>
                    <span>{devices_required}</span>
                </div>
            </div>

            <div class="bg-green-50 p-4 rounded-lg border border-green-100">
                <span class="font-semibold block text-green-800 mb-1">Example</span>
                <p>{example}</p>
            </div>
        </div>
        
        <div class="mt-8 text-center">
            <button onclick="document.getElementById('task-info-popup').style.display='none';document.getElementById('task-content').style.display='block';" 
                    class="primary-bg text-white px-8 py-3 rounded-lg font-semibold text-lg hover:bg-blue-700 transition shadow-md">
                I Understand, Start Task
            </button>
        </div>
    </div>

    <!-- Main Task Content (Hidden Initially) -->
    <div id="task-content" style="display:none;" class="max-w-4xl mx-auto">
        <div class="bg-white rounded-xl shadow-lg p-6 md:p-8">
             <div class="flex items-center justify-between mb-6 border-b pb-4">
                <h2 class="text-2xl font-bold text-gray-800">{task_name}</h2>
            ...
            <div class="mt-8 pt-6 border-t flex justify-between items-center">
                <a href="/participant-dashboard" class="text-gray-600 hover:text-gray-800 font-medium">
...
"""
    ...
    # Note: The route is handled by the dynamic route /task_<safe_name>.html
...
def parent_profile():
    """Renders the parent profile page."""
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('signin')) # Or parent_signin if exists, checking authentication is key
    
    # Ideally checking if user is parent
...
@app.route('/api/parent/profile', methods=['POST'])
def api_parent_profile():
    """API endpoint to update parent profile information."""
    try:
        data = request.get_json()
        user_id = session.get('user_id')
        if not user_id:
        ...
        # Check if email is being changed and if the new email already exists (excluding current user)
        if email:
        ...
        # If password change requested, interpret current password
        # In a real app, we MUST verify current password before changing it or email/critical info
        # For this implementation, we will check current password if provided, or if new password is being set
        
        # NOTE: The user requested "Password" field to be current password placeholder.
        # We should verify it if important changes are made or just strictly require it for password changes.
        ...
        print(f"Parent profile update error: {e}")
        return jsonify({'success': False, 'message': 'Internal server error'}), 500
...
def submit_feedback():
    """API endpoint to save user feedback for a task."""
    try:
        data = request.get_json()
        user_id = session.get('user_id')
        ...
        if not task_name or rating is None:
            return jsonify({'success': False, 'message': 'Task name and rating are required'}), 400
        ...
        query = """
            INSERT INTO task_feedback (user_id, task_name, rating, comments)
            VALUES (%s, %s, %s, %s)
        """
        cursor.execute(query, (user_id, task_name, rating, comments))
        conn.commit()
...
def submit_typing_task():
    ...
    try:
        ...
        # Get specific typing task ID if available (passed from frontend)
        specific_task_id = data.get('task_id')
        
        reference_text = None
        if specific_task_id:
            try:
                 cursor.execute("SELECT prompt FROM typing_tasks WHERE id = %s", (specific_task_id,))
                 tt_row = cursor.fetchone()
                 if tt_row:
                     reference_text = tt_row[0]
            except Exception as e:
                print(f"Error fetching reference text: {e}")

        # Calculate metrics
        metrics = calculate_typing_metrics(text, keystrokes, reference_text=reference_text)
        
        # Save final progress to typing_progress table
        cursor.execute('''
            INSERT INTO typing_progress (
                attempt_id, text, keystrokes, timer, 
                spelling_error_rate, sentence_length_variance, edit_density, insertions, deletions, 
                updated_at
            )
            ...
            ON DUPLICATE KEY UPDATE 
                ...
                timer=VALUES(timer), 
                spelling_error_rate=VALUES(spelling_error_rate), 
                sentence_length_variance=VALUES(sentence_length_variance), 
                edit_density=VALUES(edit_density), 
...
def start_reading_task():
    """Mark a reading task as started and create an attempt"""
    if 'user_id' not in session:
        return jsonify({'success': False, 'message': 'User not logged in'}), 401
    ...
    try:
        ...
        # 1. Get the Generic Task ID for "Reading Aloud Task 1"
        cursor.execute("SELECT id FROM tasks WHERE task_name = 'Reading Aloud Task 1'")
        generic_task_row = cursor.fetchone()
        
        if not generic_task_row:
             # Fallback if not found (should be there from setup SQL)
             print("Warning: 'Reading Aloud Task 1' not found in tasks table.")
             return jsonify({'success': False, 'message': 'System configuration error: Task not found'}), 500
             
        generic_generic_task_id = generic_task_row['id']
        ...
        cursor.execute("""
            ...
            FROM user_task_attempts 
            WHERE user_id = %s AND task_id = %s
        """, (session['user_id'], generic_generic_task_id))
        next_attempt = cursor.fetchone()['next_attempt']
        
        # 3. Create User Task Attempt (This is crucial for upload_video to link stats)
        cursor.execute("""
            INSERT INTO user_task_attempts (user_id, task_id, attempt_number, status, started_at)
            VALUES (%s, %s, %s, 'In Progress', NOW())
        """, (session['user_id'], generic_generic_task_id, next_attempt))
        
        # 4. Update User Task Status (High-level tracking)
        # We update the main 'Reading Aloud Task 1' status
        cursor.execute('''
            INSERT INTO user_tasks (user_id, task_name, status)
            VALUES (%s, %s, %s)
            ON DUPLICATE KEY UPDATE status = VALUES(status), updated_at = CURRENT_TIMESTAMP
...
def admin_inquiries_list():
    ...
    try:
        ...
        return jsonify({'success': False, 'message': 'Error fetching inquiries'}), 500
    finally:
        cursor.close()
        conn.close()
...
def get_student_scores(user_id):
    ...
    try:
        ...
        # 3b. Fetch Reading Stats
        try:
            cursor.execute("""
                SELECT rp.wpm, rp.pause_frequency, rp.smoothness, rp.total_words, rp.audio_duration, 
                       rp.accuracy, rp.skipped_words, rp.mispronounced_words, rp.extra_words, rp.transcribed_text,
                       uta.attempt_number
                FROM reading_progress rp
                JOIN user_task_attempts uta ON rp.attempt_id = uta.id
                WHERE uta.user_id = %s
            ...
            if reading_result:
                stats['reading_stats'] = {
                    ...
                    'pause_frequency': reading_result['pause_frequency'],
                    'smoothness': reading_result['smoothness'],
                    'total_words': reading_result['total_words'],
                    'audio_duration': reading_result['audio_duration'],
                    'accuracy': reading_result.get('accuracy', 0),
                    'skipped_words': reading_result.get('skipped_words', 0),
                    'mispronounced_words': reading_result.get('mispronounced_words', 0),
                    'extra_words': reading_result.get('extra_words', 0),
                    'transcribed_text': reading_result.get('transcribed_text', ''),
                    'attempt_number': reading_result['attempt_number']
                }
        except Exception as e:
            print(f"Error fetching reading stats: {e}")

        # 3c. Fetch Typing Stats
        try:
            cursor.execute("""
                SELECT tp.spelling_error_rate, tp.sentence_length_variance, tp.edit_density, uta.attempt_number
            ...
            if typing_result:
                stats['typing_stats'] = {
                    'spelling_error_rate': typing_result.get('spelling_error_rate', 0),
                    'sentence_length_variance': typing_result.get('sentence_length_variance', 0),
                    'edit_density': typing_result.get('edit_density', 0),
                    'attempt_number': typing_result.get('attempt_number')
                }
        except Exception as e:
            print(f"Error fetching typing stats: {e}")
...
def analyze_audio(file_path, text_content):
    """
    Analyze audio file for reading statistics:
    ...
    try:
        ...
        # Audio duration in seconds (and minutes)
        ...
        # --- WORD COUNT STRATEGY ---
        # 1. Try Speech Recognition (Best Accuracy)
        # 2. Fallback: Heuristic based on Speech Duration
        ...
        try:
            ...
            # Convert to WAV for SpeechRecognition if not already
            # pydub can export to a file-like object
            wav_io = io.BytesIO()
            ...
            # This is free and usually works for short clips
            ...
            print("SpeechRecognition library not installed. Using fallback.")
            transcribed_text = ""
        ...
        # Fallback Heuristic if SR failed
        if not recognition_success:
            print("Using Heuristic Word Count Fallback")
            ...
            total_silence_len = sum([stop - start for start, stop in silences]) # in ms
            total_silence_sec = total_silence_len / 1000.0
            speech_duration = duration_sec - total_silence_sec
            
            # Estimate words: Avg 2.5 words per second of speech
            estimated_words = speech_duration * 2.5
            observed_word_count = int(estimated_words)
            
            # Sanity check: Can't have read more words than in the text
            # But the user might repeat words? 
            # Let's cap it at total_words * 1.5 just in case
            max_words = len(text_content.split())
            if observed_word_count > max_words * 1.2:
                 observed_word_count = max_words

        # WPM Calculation
        wpm = observed_word_count / duration_min if duration_min > 0 else 0
        
        # Silence Detection (Pauses)
        # min_silence_len: in ms (e.g., 500ms = 0.5s pause)
        # silence_thresh: in dBFS (relative to max volume)
        silence_thresh = audio.dBFS - 16
        silences = silence.detect_silence(audio, min_silence_len=500, silence_thresh=silence_thresh)
        ...
        pause_frequency = num_pauses / duration_min if duration_min > 0 else 0
        
        # Smoothness Score (0-100)
        speech_duration = duration_sec - total_silence_sec
        smoothness = (speech_duration / duration_sec) * 100
        
        # Calculate Accuracy Matches
        accuracy_metrics = calculate_reading_accuracy(text_content, transcribed_text)
        
        return {
            'wpm': round(wpm, 2),
            'pause_frequency': round(pause_frequency, 2),
            'smoothness': round(smoothness, 2),
            'total_words': observed_word_count,
            'audio_duration': round(duration_sec, 2),
            'accuracy': accuracy_metrics['accuracy'],
            'skipped_words': accuracy_metrics['skipped_words'],
            'mispronounced_words': accuracy_metrics['mispronounced_words'],
            'extra_words': accuracy_metrics['extra_words'],
...
def upload_video():
    """API Endpoint to upload task recording videos."""
    if 'user_id' not in session:
       return jsonify({'success': False, 'message': 'User not logged in'}), 401
    ...
    if not task_id and not task_name:
        # Try to infer attempt from user session or latest attempt
        pass 

    try:
        user_id = session['user_id']
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{timestamp}_{user_id}_{task_id if task_id else 'unknown'}.webm"
        ...
        # If we have a task_name, get the correct generic task_id from tasks table
        target_task_id = task_id
        if task_name:
            cursor.execute("SELECT id FROM tasks WHERE task_name = %s", (task_name,))
            task_record = cursor.fetchone()
            if task_record:
                target_task_id = task_record[0]
                print(f"Resolved task_name '{task_name}' to task_id {target_task_id}")
        
        # Find the latest in-progress attempt for this task
        query_attempt = """
            SELECT id FROM user_task_attempts 
            WHERE user_id = %s AND task_id = %s 
            ORDER BY started_at DESC LIMIT 1
        """
        cursor.execute(query_attempt, (user_id, target_task_id))
        attempt = cursor.fetchone()
        
        if attempt:
            attempt_id = attempt[0]
            insert_query = "INSERT INTO video_recordings (attempt_id, filename) VALUES (%s, %s)"
            cursor.execute(insert_query, (attempt_id, filename))
            
            # Analyze Reading Task Audio
            if task_name == 'Reading Aloud Task 1':
                try:
                    # Fetch the content text from reading_tasks table
                    # The `task_id` parameter from the form corresponds to the specific reading task ID
                    cursor.execute("SELECT content FROM reading_tasks WHERE id = %s", (task_id,))
                    reading_task_record = cursor.fetchone()
                    
                    if reading_task_record:
                        text_content = reading_task_record[0]
                        print(f"Analyzing audio for task {task_id} with content length {len(text_content)}")
                        stats = analyze_audio(filepath, text_content)
                        
                        if stats:
                            cursor.execute("""
                                INSERT INTO reading_progress 
                                (attempt_id, wpm, pause_frequency, smoothness, total_words, audio_duration, 
                                 accuracy, skipped_words, mispronounced_words, extra_words, transcribed_text)
                                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                            """, (attempt_id, stats['wpm'], stats['pause_frequency'], stats['smoothness'], stats['total_words'], stats['audio_duration'],
                                  stats['accuracy'], stats['skipped_words'], stats['mispronounced_words'], stats['extra_words'], stats['transcribed_text']))
                            print(f"Reading stats saved for attempt {attempt_id}: {stats}")
                        else:
                            print("Audio analysis returned None")
                    else:
                        print(f"No reading task found for id {task_id}")
                except Exception as e:
                    print(f"Error analyzing reading task: {e}")

            conn.commit()
            cursor.close()
            conn.close()
            
            response_data = {'success': True, 'filename': filename}
            if task_name == 'Reading Aloud Task 1' and 'stats' in locals() and stats:
            ...
            cursor.close()
            conn.close()
            # If no attempt found, we still saved the file but can't link it.
            # This might happen if 'start task' wasn't called.
            return jsonify({'success': True, 'message': 'Video saved but no active attempt found to link.', 'filename': filename})

    except Exception as e:
        print(f"Video upload error: {e}")
        return jsonify({'success': False, 'message': 'Internal server error'}), 500

@app.route('/api/handle-audio-upload', methods=['POST'])
def handle_audio_upload():
    """API Endpoint to upload task recording audio (for tasks that use audio-only)."""
    if 'user_id' not in session:
       return jsonify({'success': False, 'message': 'User not logged in'}), 401
    ...
    file = request.files['audio']
    task_name = request.form.get('task_name')
    # task_id might not be sent directly or might be in form
    task_id = request.form.get('task_id')
    ...
    try:
        ...
        # Ensure uploads/audio directory exists (or just use videos for now to keep it simple, 
        # but better to separate if we want)
        # The existing code uses 'videos' for both? Let's stick to 'videos' folder for consistency 
        # or create 'audio' if preferred. Let's use 'videos' as the analysis tools expect it there usually?
        # actually analyze_audio takes a filepath.
        audio_dir = os.path.join(app.config['UPLOAD_FOLDER'], 'videos') # Keep using videos dir for now
        ...
        # Resolve task_name to task_id if needed
        target_task_id = task_id
        if task_name and not target_task_id:
            cursor.execute("SELECT id FROM tasks WHERE task_name = %s", (task_name,))
            task_record = cursor.fetchone()
            if task_record:
                target_task_id = task_record[0]
        ...
        if attempt:
            attempt_id = attempt[0]
            # Verify if we should record this file in video_recordings or a new table?
            # For now, put in video_recordings as it's the media table
            insert_query = "INSERT INTO video_recordings (attempt_id, filename) VALUES (%s, %s)"
            cursor.execute(insert_query, (attempt_id, filename))
            
            # Analyze if it's the reading task
            # For 'Reading Aloud Task 1', we might need to look up the specific reading content
            # The frontend sends `task_name`='Reading Aloud Task 1'.
            # But the specific content is in `reading_tasks` table, and we need THAT ID.
            # The frontend logic in task11.html parses URL param task_id.
            # Does it send it in the form? 
            # In task11.html submitBtn.onclick:
            # formData.append('task_name', 'Reading Aloud Task 1');
            # It DOES NOT seem to append 'task_id' (the specific reading task id).
            # We strictly need that to get the ground truth text!
            # We must update frontend to send task_id.
            # Assuming we fix frontend to send 'reading_task_id' or 'task_id' (specific):
            
            # Let's assume request.form.get('task_id') IS the specific reading task ID because
            # standard tasks have generic IDs, but reading tasks have specific IDs.
            # Actually, `target_task_id` above is the GENERIC task ID (from `tasks` table).
            # We need the SPECIFIC reading task ID to get content.
            # We will rely on a new form field 'reading_task_id' or infer from session/url if possible.
            # BEST: Update frontend to send it.
            
            specific_task_id = request.form.get('reading_task_id')
            
            if task_name == 'Reading Aloud Task 1' and specific_task_id:
                 try:
                    cursor.execute("SELECT content FROM reading_tasks WHERE id = %s", (specific_task_id,))
                    reading_task_record = cursor.fetchone()
                    ...
                    if reading_task_record:
                        ...
                        if stats:
                            ...
                            cursor.execute("UPDATE user_task_attempts SET status='Completed', completed_at=NOW() WHERE id=%s", (attempt_id,))
                            cursor.execute("""
                                INSERT INTO user_tasks (user_id, task_name, status) VALUES (%s, %s, 'Completed')
            ...
            cursor.close()
            conn.close()
            return jsonify({'success': True, 'message': 'Audio saved but no active attempt found.', 'filename': filename})
